...
export class ReportRepository {
  ...
  getAllBorrowers = async () => {
    ...
    try {
      ...
      result.rows.forEach((row) => {
        const borrower = new BorrowerDTO(
          row.id_borrower,
          row.first_name,
          row.last_name,
          row.email
  ...
  getAllOverdue = async () => {
    const queryText =
      "SELECT bb.ISBN, b.title, b.author_id, b.publication_year, b.is_available FROM books_borrowers bb INNER JOIN books b ON bb.ISBN = b.ISBN WHERE bb.borrowing_date < bb.due_date";
    try {
      const result = await pool.query(queryText);
